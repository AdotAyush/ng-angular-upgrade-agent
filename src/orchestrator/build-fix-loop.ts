...
import { BuildError, FixContext, FixResult, EnvironmentInfo, ErrorCategory } from '../types';
import { ErrorClassifier } from '../classifiers/error-classifier';
import { FixStrategyRegistry } from '../strategies/fix-strategies';
import { LLMClient, LLMGuardrails } from '../llm/llm-client';
...
export class BuildFixLoop {
  private errorClassifier: ErrorClassifier;
  private fixRegistry: FixStrategyRegistry;
  private llmClient: LLMClient;
  private agenticClient: AgenticLLMClient | null = null;
  private responseCache: LLMResponseCache | null = null;
  private schematicRunner: AngularSchematicRunner;
  private maxAttempts: number;
  private fileBackups: Map<string, string> = new Map(); // Track original file contents
  private useAgenticMode: boolean;
  private useCache: boolean;
  private useSchematics: boolean;
  private verbose: boolean;
  private schematicsRunThisSession: Set<string> = new Set(); // Track which schematics have been run

  constructor(options: BuildFixLoopOptions) {
    this.errorClassifier = new ErrorClassifier();
    this.fixRegistry = new FixStrategyRegistry();
    this.schematicRunner = new AngularSchematicRunner();
    this.llmClient = new LLMClient(options.llmApiKey, options.llmProvider || 'gemini', {
   ...
   */
  private initializeForProject(projectPath: string): void {
    // Initialize agentic client if enabled
    if (this.useAgenticMode && !this.agenticClient) {
      this.agenticClient = new AgenticLLMClient(this.llmClient, projectPath, {
        maxIterations: 10,
    ...
    if (this.useCache && !this.responseCache) {
      this.responseCache = new LLMResponseCache(projectPath, { enabled: true });
      if (this.verbose) {
  ...
  static create(
    ...
    llmProvider: 'bedrock' | 'gemini' = 'gemini',
    llmOptions?: {
      model?: string;
      awsRegion?: string;
      awsSecretKey?: string;
  ...
  async execute(
    ...
    const resolvedErrors: BuildError[] = [];
    const appliedFixes: FixResult[] = [];
    let attempts = 0;
    let unresolvedErrors: BuildError[] = [];
    let previousErrorCount: number | null = null;
  ...
  private async tryLLMFix(
    error: BuildError, 
    context: FixContext, 
    buildOutput?: string
  ): Promise<FixResult & { fromCache?: boolean }> {
    ...
    if (!error.file) {
      ...
      if (this.useAgenticMode && this.agenticClient && buildOutput) {
        ...
        try {
          const agenticResult = await this.agenticClient.requestFix(
            error,
            `Project: ${context.projectPath}, Target: Angular ${context.targetVersion}`,
            buildOutput
          );
    ...
    // Validate request
    const validation = LLMGuardrails.validateRequest(request);
    if (!validation.valid) {
      return {
        success: false,
    ...
    try {
      ...
      // Validate response
      const responseValidation = LLMGuardrails.validateResponse(response);
  ...
  private async findRelevantContext(
    error: BuildError,
    projectPath: string
  ): Promise<{ files: string[]; searchInfo: string }> {
    const fs = await import('fs');
    ...
    for (const dir of searchDirs) {
      ...
      try {
        ...
        if (relevantFiles.length >= 3) break; // Limit to first 3 files
      } catch {
   ...
   */
  private async recursiveFileSearch(
    dir: string,
    searchTerms: string[],
    readdir: any,
    readFile: any,
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (stats.isDirectory()) {
          ...
          const subMatches = await this.recursiveFileSearch(
            fullPath,
            searchTerms,
            readdir,
            readFile,
        ...
        if (matches.length >= 3) break; // Limit results
      }
    } catch {
      // Ignore errors
    }
   ...
   */
  private isSchematicFixableError(error: BuildError): boolean {
  ...
  private async trySchematicFix(
    ...
    try {
      ...
      const { stdout, stderr } = await execAsync(
        `npx @angular/cli@${targetVersion} generate ${schematicToRun} --defaults`,
        {
          cwd: projectPath,
      ...
      if (packageToUpdate) {
        ...
        const { stdout: updateOut, stderr: updateErr } = await execAsync(
          `npx @angular/cli@${targetVersion} update ${packageToUpdate} --migrate-only --allow-dirty --force`,
          {
            cwd: projectPath,
            maxBuffer: 50 * 1024 * 1024,
          }
   ...
   */
  private determineSchematic(error: BuildError): string | null {
   ...
   */
  private getPackageForSchematic(schematic: string): string | null {
  ...
  async verifyBuildAndTests(
    projectPath: string,
    environment: EnvironmentInfo
  ): Promise<{ buildPassed: boolean; testsPassed: boolean }> {
    const buildResult = await this.runBuild(projectPath, environment);
