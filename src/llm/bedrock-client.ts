...
import { LLMRequest, LLMResponse, FileChange } from '../types';
...
export class BedrockClient {
  ...
  constructor(
    ...
    if (accessKeyId && secretAccessKey) {
      ...
      if (envSecret) {
        ...
        throw new Error(
          'AWS Secret Access Key is required. Provide via --aws-secret-key or set AWS_SECRET_ACCESS_KEY environment variable'
        );
      }
    }
  ...
  private resolveModelId(modelId: string | undefined, region: string): string {
    ...
    const inferenceProfiles: Record<string, string> = {
   ...
   */
  private sanitizeForSigning(text: string): string {
    return text
  ...
  private buildPrompts(request: LLMRequest): { systemPrompt: string; userPrompt: string } {
...
Provide SEARCH/REPLACE blocks to fix ONLY the problematic code. Do NOT rewrite the entire file.`;
   ...
   */
  private extractErrorContext(fileContent: string, errorLine: number, contextLines: number = 30): string {
  ...
  private parseResponse(content: string, request: LLMRequest): LLMResponse {
  ...
  private validateFix(fixedCode: string, request: LLMRequest): boolean {
