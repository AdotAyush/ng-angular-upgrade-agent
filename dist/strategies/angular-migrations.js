...
exports.RxJSImportStrategy = exports.RouterMigrationStrategy = exports.HttpClientMigrationStrategy = void 0;
 ...
 */
class HttpClientMigrationStrategy {
    name = 'HttpClientMigrationStrategy';
    category = types_1.ErrorCategory.IMPORT;
    isDeterministic = true;
    canHandle(error) {
        return (error.message.includes('HttpClientModule') ||
        ...
        try {
            ...
            if (!content.includes('@angular/common/http')) {
                ...
                if (importMatch) {
                    ...
                    newContent = content.replace(lastImport, `${lastImport}import { provideHttpClient, withInterceptorsFromDi } from '@angular/common/http';\n`);
                }
            }
            else {
                // Update existing import
                newContent = content.replace(/import\s*{([^}]+)}\s*from\s*['"]@angular\/common\/http['"]/, (match, imports) => {
            ...
            if (providersMatch) {
                ...
                if (!providers.includes('provideHttpClient')) {
                    const newProviders = providers.trim()
                        ...
                        : `\n    provideHttpClient(withInterceptorsFromDi())\n  `;
                    newContent = newContent.replace(/providers:\s*\[([\s\S]*?)\]/, `providers: [${newProviders}]`);
                }
            }
            ...
            if (fs.existsSync(appModulePath)) {
                ...
                moduleContent = moduleContent.replace(/import\s*{\s*HttpClientModule\s*}\s*from\s*['"]@angular\/common\/http['"];?\n/, '');
...
exports.HttpClientMigrationStrategy = HttpClientMigrationStrategy;
/**
...
class RouterMigrationStrategy {
    ...
    isDeterministic = true;
    canHandle(error) {
        return (error.message.includes('RouterModule') ||
    ...
    async apply(error, context) {
        try {
            ...
            if (!content.includes('@angular/router')) {
                ...
                if (importMatch) {
                    const lastImport = importMatch[0];
                    newContent = content.replace(lastImport, `${lastImport}import { provideRouter } from '@angular/router';\nimport { routes } from './app.routes';\n`);
                }
            }
            // Add to providers
            const providersMatch = newContent.match(/providers:\s*\[([\s\S]*?)\]/);
...
exports.RouterMigrationStrategy = RouterMigrationStrategy;
/**
 * Auto-imports common RxJS operators
 */
class RxJSImportStrategy {
    ...
    canHandle(error) {
        const operatorMatch = this.rxjsOperators.some(op => error.message.includes(`'${op}'`) || error.message.includes(`"${op}"`));
        return operatorMatch && (error.message.includes('Cannot find name') ||
    ...
    async apply(error, context) {
        ...
        try {
            ...
            const missingOperator = this.rxjsOperators.find(op => error.message.includes(`'${op}'`) || error.message.includes(`"${op}"`));
            if (!missingOperator) {
            ...
            if (lastImportMatch) {
                newContent = content.replace(lastImportMatch[1], `${lastImportMatch[1]}${importStatement}`);
            }
            else {
...
exports.RxJSImportStrategy = RxJSImportStrategy;
//# sourceMappingURL=angular-migrations.js.map