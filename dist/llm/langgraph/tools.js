...
exports.executeTool = exports.TOOL_DEFINITIONS = exports.findUsagesTool = exports.analyzeRuntimeErrorTool = exports.checkPackageTool = exports.runCommandTool = exports.listFilesTool = exports.searchCodeTool = exports.readFileTool = void 0;
...
async function searchCodeTool(args, context) {
    try {
        ...
        try {
            const { stdout } = await execAsync(`rg ${caseSensitiveFlag} --glob "${filePattern}" -n "${args.pattern.replace(/"/g, '\\"')}" .`, {
            ...
            if (isWindows) {
                const { stdout } = await execAsync(`findstr /S /N ${args.caseSensitive ? '' : '/I'} "${args.pattern}" ${filePattern}`, {
                    cwd: context.projectPath,
                    maxBuffer: 2 * 1024 * 1024,
                    timeout: context.commandTimeout ?? 30000,
                });
                ...
                const { stdout } = await execAsync(`grep -rn ${caseSensitiveFlag} --include="${filePattern}" "${args.pattern.replace(/"/g, '\\"')}" .`, {
                    cwd: context.projectPath,
                    maxBuffer: 2 * 1024 * 1024,
                    timeout: context.commandTimeout ?? 30000,
                });
 ...
 */
async function listFilesTool(args, context) {
    try {
 ...
 */
async function runCommandTool(args, context) {
    try {
        const cwd = args.cwd
 ...
 */
async function checkPackageTool(args, context) {
    try {
        ...
        const isBrowserIncompatible = types_1.BROWSER_INCOMPATIBLE_PACKAGES.has(packageName);
...
exports.checkPackageTool = checkPackageTool;
/**
 * Tool: Analyze runtime error
 */
async function analyzeRuntimeErrorTool(args, context) {
    ...
    for (const pattern of types_1.RUNTIME_ERROR_PATTERNS) {
    ...
    if (stackTrace) {
        ...
        if (fileMatches) {
            diagnosis.affectedFiles = [...new Set(fileMatches.map((m) => {
        ...
        if (nodeModuleMatches) {
            const packages = [...new Set(nodeModuleMatches.map((m) => m.replace('node_modules/', '')))];
            for (const pkg of packages) {
                if (types_1.BROWSER_INCOMPATIBLE_PACKAGES.has(pkg)) {
...
exports.analyzeRuntimeErrorTool = analyzeRuntimeErrorTool;
/**
 * Tool: Find usages of a package or import
 */
async function findUsagesTool(packageName, context) {
    try {
        ...
        for (const pattern of importPatterns) {
            const searchResult = await searchCodeTool({ pattern, filePattern: '*.{ts,js,tsx,jsx}' }, context);
...
exports.findUsagesTool = findUsagesTool;
/**
...
async function executeTool(toolName, args, context) {
    switch (toolName) {
        ...
        case 'searchCode':
            return searchCodeTool(args, context);
        case 'listFiles':
            return listFilesTool(args, context);
        case 'runCommand':
            return runCommandTool(args, context);
        case 'checkPackage':
            return checkPackageTool(args, context);
        case 'analyzeRuntimeError':
            return analyzeRuntimeErrorTool(args, context);
        case 'proposeChanges':
