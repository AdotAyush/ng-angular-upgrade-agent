...
(function (PackageCategory) {
    PackageCategory["FRAMEWORK_RUNTIME"] = "FRAMEWORK_RUNTIME";
    PackageCategory["FRAMEWORK_TOOLING"] = "FRAMEWORK_TOOLING";
    PackageCategory["LANGUAGE_COMPILER"] = "LANGUAGE_COMPILER";
    PackageCategory["REACTIVE_RUNTIME"] = "REACTIVE_RUNTIME";
    PackageCategory["LINT_STACK"] = "LINT_STACK";
    PackageCategory["ENVIRONMENT_TYPES"] = "ENVIRONMENT_TYPES";
    PackageCategory["UI_LIBRARY"] = "UI_LIBRARY";
    PackageCategory["THIRD_PARTY"] = "THIRD_PARTY";
})(PackageCategory || (PackageCategory = {}));
// Version range strategies
var VersionStrategy;
(function (VersionStrategy) {
    VersionStrategy["EXACT"] = "EXACT";
    VersionStrategy["TILDE"] = "TILDE";
    VersionStrategy["CARET"] = "CARET";
    VersionStrategy["RANGE"] = "RANGE";
...
class DependencyResolver {
    ...
    async fetchPackageInfo(packageName) {
        ...
        try {
            const response = await (0, node_fetch_1.default)(`${this.npmRegistry}/${packageName}`);
            if (!response.ok)
                return null;
     ...
     */
    async fetchVersionInfo(packageName, version) {
        try {
            const response = await (0, node_fetch_1.default)(`${this.npmRegistry}/${packageName}/${version}`);
            if (!response.ok)
                return null;
     ...
     */
    formatVersion(version, strategy) {
    ...
    async resolveDependencies(packageJson, targetAngularVersion) {
        const resolutions = [];
        const conflicts = [];
        const stalePackages = [];
        ...
        const thirdPartyResolutions = await this.resolveThirdPartyAngularLibraries(packageJson, targetAngularVersion);
        ...
        const { resolved, unresolvedConflicts } = await this.iterativeConstraintResolution(resolutions, packageJson, targetAngularVersion);
        console.log('└──────────────────────────────────────────────────────────────────\n');
     ...
     */
    async resolveAngularEslint(packageJson, majorVersion, coreVersion, resolutions, conflicts) {
        const allDeps = { ...packageJson.dependencies, ...packageJson.devDependencies };
        ...
        const eslintVersion = await this.resolveLatestStableMajorVersion('@angular-eslint/schematics', majorVersion);
        if (!eslintVersion) {
     ...
     */
    async resolveNodeTypes(packageJson, majorVersion, resolutions, conflicts) {
        const allDeps = { ...packageJson.dependencies, ...packageJson.devDependencies };
     ...
     */
    extractMinNodeMajor(constraint) {
     ...
     */
    async resolveAngularMaterial(packageJson, majorVersion, coreVersion, resolutions, conflicts) {
        const allDeps = { ...packageJson.dependencies, ...packageJson.devDependencies };
        console.log('  Resolving @angular/material...');
        // Find Material version compatible with this Angular version
        const materialVersion = await this.findCompatibleVersionByPeerDep('@angular/material', '@angular/core', coreVersion);
        if (materialVersion) {
     ...
     */
    async findCompatibleVersionByPeerDep(packageName, peerDepName, targetVersion) {
     ...
     */
    async resolveLatestVersionSatisfying(packageName, constraint) {
        const pkgInfo = await this.fetchPackageInfo(packageName);
        if (!pkgInfo)
            return null;
        const versions = Object.keys(pkgInfo.versions)
     ...
     */
    async resolveLatestStableMajorVersion(packageName, majorVersion) {
        const pkgInfo = await this.fetchPackageInfo(packageName);
        if (!pkgInfo)
            return null;
        const versions = Object.keys(pkgInfo.versions)
            ...
            if (parsed.prerelease.length > 0)
                return false; // Skip prereleases
            return parsed.major === majorVersion;
        })
            .sort((a, b) => semver.compare(a, b));
        return versions.length > 0 ? versions[versions.length - 1] : null;
    }
    getAngularPackages(packageJson) {
        const allDeps = {
            ...packageJson.dependencies,
            ...packageJson.devDependencies,
        };
        return Object.keys(allDeps).filter(pkg => pkg.startsWith('@angular/') || pkg.startsWith('@angular-devkit/') || pkg.startsWith('@schematics/'));
    }
    async resolveTypescript(compatibleVersions) {
    ...
    async getLatestPatchVersion(packageName, minorVersion) {
        try {
            const response = await (0, node_fetch_1.default)(`${this.npmRegistry}/${packageName}`);
            const data = await response.json();
            const versions = Object.keys(data.versions);
            const matching = versions.filter(v => v.startsWith(`${minorVersion}.`));
     ...
     */
    async iterativeConstraintResolution(initialResolutions, packageJson, targetAngularVersion) {
        ...
        for (const [pkg, constraints] of constraintMap.entries()) {
            ...
            if (intersectionVersion) {
                ...
                unresolvedConflicts.push(`❌ INCOMPATIBLE: ${pkg} has conflicting requirements: ${conflictDetail}`);
        ...
        if (remainingConflicts.length === 0) {
            ...
            for (const conflict of remainingConflicts) {
                unresolvedConflicts.push(`${conflict.package}@${conflict.currentVersion} requires ${conflict.peerPackage}@${conflict.peerConstraint}, ` +
     ...
     */
    async findVersionSatisfyingAll(packageName, constraints) {
        const pkgInfo = await this.fetchPackageInfo(packageName);
        if (!pkgInfo)
            return null;
        const versions = Object.keys(pkgInfo.versions)
     ...
     */
    async findAllPeerConflicts(resolutions) {
        const conflicts = [];
        for (const resolution of resolutions) {
            try {
                const version = resolution.targetVersion.replace(/[\^~]/g, '');
                const response = await (0, node_fetch_1.default)(`${this.npmRegistry}/${resolution.package}/${version}`);
                if (!response.ok)
                    continue;
                const data = await response.json();
                ...
                for (const [peerPackage, constraint] of Object.entries(peerDeps)) {
                    ...
                    if (peerResolution) {
                        ...
                        try {
                            if (!semver.satisfies(peerVersion, constraint)) {
                                conflicts.push({
     ...
     */
    async resolveConflict(conflict, resolutions, packageJson, targetAngularVersion) {
        ...
        if (peerResolutionIndex === -1)
            return false;
        // Try to find a version of the peer package that satisfies the constraint
        try {
            const response = await (0, node_fetch_1.default)(`${this.npmRegistry}/${conflict.peerPackage}`);
            if (!response.ok)
                return false;
            const data = await response.json();
            const versions = Object.keys(data.versions)
                ...
                try {
                    if (semver.satisfies(version, conflict.peerConstraint)) {
                        ...
                        if (angularPeerDeps['@angular/core']) {
                            isAngularCompatible = semver.satisfies(targetAngularVersion, angularPeerDeps['@angular/core']);
                        }
                        if (isAngularCompatible) {
                            // Check if this version won't break other existing resolutions
                            const wouldBreakOthers = await this.wouldBreakOtherResolutions(conflict.peerPackage, version, resolutions, peerResolutionIndex);
                            if (!wouldBreakOthers) {
     ...
     */
    async wouldBreakOtherResolutions(packageName, newVersion, resolutions, skipIndex) {
    ...
    async resolveThirdPartyAngularLibraries(packageJson, targetAngularVersion) {
        const resolutions = [];
        const conflicts = [];
        const allDeps = {
            ...packageJson.dependencies,
        ...
        const thirdPartyPackages = Object.keys(allDeps).filter(pkg => !pkg.startsWith('@angular/') &&
        ...
        for (const pkg of thirdPartyPackages) {
            try {
                ...
                if (compatibility_1.PRIVATE_PACKAGE_URLS[pkg]) {
                    const url = compatibility_1.PRIVATE_PACKAGE_URLS[pkg];
                    resolutions.push({
                        package: pkg,
                ...
                if (compatibility_1.PRIVATE_PACKAGE_VERSIONS[pkg]) {
                    const privateVersionMap = compatibility_1.PRIVATE_PACKAGE_VERSIONS[pkg];
                ...
                const compatibleVersion = await this.resolvePeerDependencyCompatibleVersion(pkg, targetAngularVersion, process.version.replace('v', ''));
                ...
                if (this.llmClient) {
                    const response = await (0, node_fetch_1.default)(`${this.npmRegistry}/${pkg}`);
                    if (response.ok) {
                        const data = await response.json();
                        const aiVersion = await this.aiAssistedVersionResolution(pkg, allDeps[pkg].replace(/[\^~]/g, ''), targetAngularVersion, data);
                        if (aiVersion) {
                            resolutions.push({
                                package: pkg,
                                currentVersion: allDeps[pkg],
                ...
                conflicts.push(`❌ ${pkg} has no compatible version found for Angular ${targetAngularVersion}`);
            }
            catch (error) {
     ...
     */
    async resolvePeerDependencyCompatibleVersion(packageName, targetAngularVersion, nodeVersion) {
        ...
        if (compatibility_1.PRIVATE_PACKAGE_VERSIONS[packageName]) {
            return null;
        }
        try {
            const response = await (0, node_fetch_1.default)(`${this.npmRegistry}/${packageName}`);
    ...
    async findCompatibleVersion(packageName, packageData, targetAngularMajor) {
    ...
    async aiAssistedVersionResolution(packageName, currentVersion, targetAngularVersion, packageData) {
        if (!this.llmClient)
            return null;
        try {
...
Response format (JSON only):
{
  ...
  "reasoning": "brief explanation"
    ...
    async applyResolutions(packageJsonPath, packageJson, resolutions) {
        for (const resolution of resolutions) {
